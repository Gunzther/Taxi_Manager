public class ThaiCustomer implements Customer {
    ...
    public double calculateFare(){
        ...
        double price = 0;
        int loop = 0;
        double 	priceRate = 5.5;// 5.5 bath in the first 10 km.
    ...
    public String toString() {
        return String.format("User: %s %s\nId: %s\nAge: %s\nGender: %s\nNationality: %s\n------------------------\n" +
                        "Date: %s\nStart: %s (%s)\nTerminal: %s (%s)\n",
                this.firstName, this.lastName, this.id, this.age, this.gender, this.nationality, this.date,
    ...
    public static class Builder {
        ...
        public Customer build() {
            Customer customer = new ThaiCustomer(this);
            return customer;
        }
    }
}
